...
!(function (t, i) {
    ...
    var n = e && document.createElement('div').style,
        r = (function () {
            if (!e) return !1;
            var t = { standard: 'transform', webkit: 'webkitTransform', Moz: 'MozTransform', O: 'OTransform', ms: 'msTransform' };
    ...
    function W(t, i, e, s, o, n, r, h) {
        var a = t - i,
            ...
            f = u ? 4 : 15,
            g = t + (l / c) * (a < 0 ? -1 : 1);
        return u && p && (g = h._findNearestValidWheel(g).y), g < s ? ((g = n ? Math.max(s - n / 4, s - (n / f) * l) : s), (m = d)) : o < g && ((g = n ? Math.min(o + n / 4, o + (n / f) * l) : o), (m = d)), { destination: Math.round(g), duration: m };
    }
    function k() {}
    var z = e
    ...
    (L.prototype.handleEvent = function (t) {
        ...
        (L.prototype.fade = function (t, i) {
            ...
            if (!i || this.visible) {
                var s = t ? 250 : 500;
                (t = t ? '1' : '0'),
                    (this.wrapperStyle[_.transitionDuration] = s + 'ms'),
    ...
    function J(t, i) {
        var e = this;
        (this.options = i),
            C('function' == typeof this.options.createTombstone, 'Infinite scroll need createTombstone Function to create tombstone'),
            C('function' == typeof this.options.fetch, 'Infinite scroll need fetch Function to fetch new data.'),
            C('function' == typeof this.options.render, 'Infinite scroll need render Function to render each item.'),
    ...
    return (
        ...
        (N.prototype.refresh = function () {
            ...
            s ? ((this.items = this.scroller.children), this._checkWheelAllDisabled(), (this.options.itemHeight = this.itemHeight = this.items.length ? this.scrollerHeight / this.items.length : 0), void 0 === this.selectedIndex && (this.selectedIndex = s.selectedIndex || 0), (this.options.startY = -this.selectedIndex * this.itemHeight), (this.maxScrollX = 0), (this.maxScrollY = -this.itemHeight * (this.items.length - 1))) : ((this.maxScrollX = this.wrapperWidth - this.scrollerWidth), this.options.infinity || (this.maxScrollY = this.wrapperHeight - this.scrollerHeight), this.maxScrollX < 0 ? ((this.maxScrollX -= this.relativeX), (this.minScrollX = -this.relativeX)) : 1 < this.scale && ((this.maxScrollX = this.maxScrollX / 2 - this.relativeX), (this.minScrollX = this.maxScrollX)), this.maxScrollY < 0 ? ((this.maxScrollY -= this.relativeY), (this.minScrollY = -this.relativeY)) : 1 < this.scale && ((this.maxScrollY = this.maxScrollY / 2 - this.relativeY), (this.minScrollY = this.maxScrollY))),
        ...
        (R.prototype._end = function (t) {
            if (this.enabled && !this.destroyed && Y[t.type] === this.initiated) {
                (this.initiated = !1), this.options.preventDefault && !M(t.target, this.options.preventDefaultException) && t.preventDefault(), this.options.stopPropagation && t.stopPropagation(), this.trigger('touchEnd', { x: this.x, y: this.y }), (this.isInTransition = !1);
                        ...
                        if (this._events.flick && n < this.options.flickLimitTime && r < this.options.flickLimitDistance && h < this.options.flickLimitDistance) this.trigger('flick');
                            ...
                            if (this.options.momentum && n < this.options.momentumLimitTime && (h > this.options.momentumLimitDistance || r > this.options.momentumLimitDistance)) {
                                ...
                                var m = (-1 === this.directionX && p) || (1 === this.directionX && d) ? this.wrapperWidth : 0,
                                    ...
                                    g = this.hasHorizontalScroll ? W(this.x, this.startX, n, this.maxScrollX, this.minScrollX, m, this.options, this) : { destination: i, duration: 0 },
                                    v = this.hasVerticalScroll ? W(this.y, this.startY, n, this.maxScrollY, this.minScrollY, f, this.options, this) : { destination: e, duration: 0 };
                            ...
                            this.options.wheel && (this.selectedIndex = this._findNearestValidWheel(this.y).index), this.trigger('scrollEnd', { x: this.x, y: this.y });
                        }
                    }
            }
        }),
            ...
            (this.isAnimating = !0),
                ...
                (function t() {
                    ...
                    var e = l((i = (i - f) / a)),
                        ...
                        o = (h - d) * e + d,
                        n = (m - u) * e + u;
                    c._translate(s, o, n), c.isAnimating && (c.animateTimer = z(t)), 3 === c.options.probeType && c.trigger('scroll', { x: c.x, y: c.y });
                })();
        }),
            ...
            if ((this.options.wheel && (i = this._findNearestValidWheel(i).y), t !== this.x || i !== this.y))
                if (((this.isInTransition = this.options.useTransition && 0 < e && (this.x !== t || this.y !== i)), !e || this.options.useTransition)) {
                    if ((this._transitionTimingFunction(s.style), this._transitionTime(e), this._translate(t, i), e && 3 === this.options.probeType && this._startProbe(), !e && !o)) {
                        ...
                        this.trigger('scroll', { x: t, y: i }), (this._reflow = document.body.offsetHeight), this.resetPosition(this.options.bounceTime, I.bounce) || this.trigger('scrollEnd', { x: t, y: i });
                    }
        ...
        (R.prototype.getComputedPosition = function () {
            var t = window.getComputedStyle(this.scroller, null),
                i = void 0,
                e = void 0;
            return (e = this.options.useTransform ? ((i = +((t = t[_.transform].split(')')[0].split(', '))[12] || t[4])), +(t[13] || t[5])) : ((i = +t.left.replace(/[^-\d.]/g, '')), +t.top.replace(/[^-\d.]/g, ''))), { x: i, y: e };
        }),
        (R.prototype.stop = function () {
            if (this.options.useTransition && this.isInTransition) {
                (this.isInTransition = !1), H(this.probeTimer);
                var t = this.getComputedPosition();
                this._translate(t.x, t.y), this.options.wheel ? (this.target = this.items[this._findNearestValidWheel(t.y).index]) : this.trigger('scrollEnd', { x: this.x, y: this.y }), (this.stopFromTransition = !0);
        ...
        ((U = K).prototype._initSnap = function () {
            ...
            var g = f.el;
            'string' == typeof g && (g = this.scroller.querySelectorAll(g)),
                this.on('refresh', function () {
                    if (((m.pages = []), m.wrapperWidth && m.wrapperHeight && m.scrollerWidth && m.scrollerHeight)) {
                        var t = f.stepX || m.wrapperWidth,
                            ...
                            l = void 0,
                            c = void 0;
                        if (g) for (h = g.length, l = -1; r < h; r++) (c = X(g[r])), (0 === r || c.left <= X(g[r - 1]).left) && ((a = 0), l++), m.pages[a] || (m.pages[a] = []), (e = Math.max(-c.left, m.maxScrollX)), (s = Math.max(-c.top, m.maxScrollY)), (o = e - Math.round(c.width / 2)), (n = s - Math.round(c.height / 2)), (m.pages[a][l] = { x: e, y: s, width: c.width, height: c.height, cx: o, cy: n }), e > m.maxScrollX && a++;
                        else
        ...
        (U.prototype._goToPage = function (t) {
            ...
            if (n && this.pages && this.pages.length && ((s = s || n.easing || I.bounce), t >= this.pages.length ? (t = this.pages.length - 1) : t < 0 && (t = 0), this.pages[t])) {
                ...
                var r = this.pages[t][i].x,
                    h = this.pages[t][i].y;
                (e = void 0 === e ? n.speed || Math.max(Math.max(Math.min(Math.abs(r - this.x), 1e3), Math.min(Math.abs(h - this.y), 1e3)), 300) : e), (this.currentPage = { x: r, y: h, pageX: t, pageY: i }), this.scrollTo(r, h, e, s, o);
            }
        }),
        ...
        ((B = K).prototype._initScrollbar = function () {
            ...
            this.indicators = [];
            var r = void 0;
            this.options.scrollX && ((r = { el: O('horizontal'), direction: 'horizontal', fade: s, interactive: n }), this._insertScrollBar(r.el), this.indicators.push(new L(this, r))),
                this.options.scrollY && ((r = { el: O('vertical'), direction: 'vertical', fade: s, interactive: n }), this._insertScrollBar(r.el), this.indicators.push(new L(this, r))),
                this.on('refresh', function () {
        ...
        (G.prototype._zoom = function (t) {
            if (this.enabled && !this.destroyed && Y[t.type] === this.initiated) {
                ...
                var o = this.options.zoom,
                    ...
                    h = o.max,
                    a = void 0 === h ? 4 : h;
                s < r ? (s = 0.5 * r * Math.pow(2, s / r)) : a < s && (s = 2 * a * Math.pow(0.5, a / s));
